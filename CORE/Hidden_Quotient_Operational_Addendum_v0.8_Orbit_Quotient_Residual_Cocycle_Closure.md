...
This is the concrete stationary-autonomy counterexample required by §18: the slip witness erased phase that the next transition still uses. The repair is to retain $\theta_n$ itself, or another sufficient phase state. Recording only the constant circle increment $[\theta_{n+1}-\theta_n]=[\alpha]$ in $\mathbb R/\mathbb Z$ does not repair the witness. The exact classification is therefore:

$$
\boxed{
\text{driven quotient cocycle: exact; residual descent: failed; closure status: UNRESOLVED.}
}
$$
...
screen_residual_cocycle:
  ...
  residual_only_descent_status: unknown
  residual_only_closure_status: unknown
  closure_counterexample_ids: []
  sufficient_state_refinement: null
formal_register:
  factors: []
  formal_state_count: null
...
28. **Slip-autonomy holdout.** Supply the golden-screen indices $1$ and $8$. The system must preserve equal current slips with unequal next slips and refuse a stationary slip-only law.
29. **Phase-refinement holdout.** Offer the constant circle increment without the phase value. The system must continue to mark residual-only closure unresolved.
...
**Explicit non-claims:** v0.8 does not claim a hidden controller, physical sector, platform implementation, moving lattice, common clock, $C_{72}$ geometry, empirical event class, causal mechanism, or autonomous slip process. Exact quotient arithmetic is not evidence that the quotient is physically instantiated.

**Version seal:** v0.8 succeeds v0.7 without rewriting the v0.7 return-residual companion or the v0.6 eligibility/response instrument. It narrowly supersedes the inherited use of *autonomous* for a time-indexed factor, and adds exact $C_2$ and $C_3$ quotient witnesses, typed orbit and slip residuals, accumulated cocycles, a located residual-descent counterexample, and a conditional formal $72$ register with an explicit cardinality-and-evidence firewall.
