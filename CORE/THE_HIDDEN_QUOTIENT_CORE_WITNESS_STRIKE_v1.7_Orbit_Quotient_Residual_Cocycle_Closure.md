...
**v1.7 scope of change.** Part I and Bridges I–VI remain sovereign. Bridge VII gains a finite worked specimen: the exact $C_2$ orbit quotient of the conditional $C_6\times C_4$ weave, the exact $C_3$ strand quotient of the fixed $39$-screen, typed sector-switch and slip residuals, a located failure of residual-only descent, and a conditional formal $72$ register. It also distinguishes time-indexed residual factors from a single autonomous factor. The Bridge VI YAML version identifiers are quoted so serialization preserves their exact labels. These additions sharpen state accounting and falsifiability; they do not validate a platform geometry or alter the null frozen-bank/moving-lattice verdict.
...
Indeed $s_1^{(39)}=s_8^{(39)}=2$, so even the current pair $(s_n^{(39)},\sigma_n^{(39)})$ is insufficient. There is no stationary $F_\sigma:\{0,1\}\to\{0,1\}$ with $\sigma_{n+1}^{(39)}=F_\sigma(\sigma_n^{(39)})$ on this common orbit domain. Retain $\theta_n$ itself, or an equivalent sufficient phase state; retaining only the constant circle increment $[\alpha]\in\mathbb R/\mathbb Z$ does not restore descent.
