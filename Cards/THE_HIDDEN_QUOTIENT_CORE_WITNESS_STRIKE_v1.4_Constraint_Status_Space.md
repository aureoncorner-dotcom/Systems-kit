...
### Multiplication representation over arbitrary measures, with two declared readings and five typed companion bridges

**Status:** unified master edition — family-built  
**Version:** 1.4 — v1.0 mathematical core preserved; retained-context, Open-Seam, constraint-demotion, and constraint-status companions added; Seat 2, Seat 5, and governing OMNIBUS v5.57 clauses retained  
...
Five bridge cards follow the mathematical core. Bridge I separates visibility from completion. Bridge II states the retained-context upgrade in typed form. Bridge III extends local transport into a directed atlas of typed seams, paths, alternate routes, and loop returns. Bridge IV supplies an event-level audit for constraint demotion across independently located empirical families. Bridge V completes the post-event binding-by-route state space and makes macrostate labels explicit projections. Bridges II–V are companions to the theorem layer, not additional theorems, new seats, or backward alterations of Part I.
...
The embedded Seat 2 and Seat 5 cards retain their local v1.0 labels because their bodies are unchanged; v1.3 is the version of this containing master.
...
The first coordinate records a nontrivial collapse. The second says the seam determines admission, weight, adjudication, or closure. The third says correction does not bind into the governing map. The fourth says no alternate path is presently available.
...
**Status:** empirical systems companion to Bridge III  
...
The current corpus contains three independently located lanes and one structural fingerprint. Their denominators and dependence structures differ and must not be pooled as though they were one sample.
...
Its recurrence across lexically different substituted propositions supports a structural comparison. It does not identify continuity, occupant, intention, motive, or shared implementation.
...
| $A^+$ | $O=1$, $E\uparrow$, $B=\text{UNKNOWN or unresolved}$ | annexation or interposition candidate without demonstrated binding loss |
...
Agency expansion normally enters as $A^+$ until the before-and-after governing status of the supplied constraint is demonstrated. Correction nonbinding establishes $B\downarrow$ in direct specimens but does not establish $E\uparrow$ without a located route. Review-state events remain candidates until each coordinate is coded.

### 5. Deletion test, controls, and dependence

For the deletion test, remove only the added layer while holding the substantive answer fixed. If the object and task survive and the constraint-governed route is restored, the added layer is structurally separable and route-relevant. The result does not identify why the layer appeared.
...
For v1.4 records, $A^+$ denotes the resolved route-annexation cell $(B_{\mathrm{after}},E_{\mathrm{after}})=(1,1)$. This refines the provisional Bridge IV candidate use of $A^+$ without rewriting v1.3 records.
...
- At least four of the eight frozen review-state candidates were codable as the full $D^+$ state in the retrospective pass. The remaining four were unresolved at one or more required coordinates in that pass and therefore remain unresolved, not negative.
...
- that four retrospectively codable review episodes estimate a population prevalence;
